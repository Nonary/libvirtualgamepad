...
/*
 * USB host+gadget loopback for Vibeshine DualSense emulation.
 * Derived from Linux dummy_hcd.c (David Brownell / Alan Stern) with
 * isochronous transfers enabled so DualSense USB audio can enumerate.
 */
...
#define DRIVER_DESC	"Vibeshine DualSense USB loopback"
...
static const char	driver_name[] = "vibeshine_ds5_hcd";
static const char	driver_desc[] = "Vibeshine DualSense USB loopback";

static const char	gadget_name[] = "vibeshine_ds5_udc";
...
/* Isochronous endpoints are required for DualSense USB audio. */
#define TYPE_BULK_OR_INT	(USB_EP_CAPS_TYPE_BULK | USB_EP_CAPS_TYPE_INT)
#define TYPE_ALL		(TYPE_BULK_OR_INT | USB_EP_CAPS_TYPE_ISO)
...
*/
	...
	EP_INFO("ep-aout",
		USB_EP_CAPS(TYPE_ALL, USB_EP_CAPS_DIR_OUT)),
	EP_INFO("ep-bin",
		USB_EP_CAPS(TYPE_ALL, USB_EP_CAPS_DIR_IN)),
	EP_INFO("ep-cout",
		USB_EP_CAPS(TYPE_ALL, USB_EP_CAPS_DIR_OUT)),
	EP_INFO("ep-dout",
		USB_EP_CAPS(TYPE_ALL, USB_EP_CAPS_DIR_OUT)),
...
restart:
	list_for_each_entry_safe(urbp, tmp, &dum_hcd->urbp_list, urbp_list) {
		...
		case PIPE_ISOCHRONOUS: {
			...
			u64 interval = (u64)max(urb->interval, 1) *
				(dum->gadget.speed >= USB_SPEED_HIGH ? 125000 : 1000000);
			unsigned int len = 0;

			/* One USB packet per endpoint interval, not an entire URB
			 * every 125-us scheduler tick. Completing every descriptor
			 * into one gadget request both sped up ALSA's clock and
			 * silently truncated audio after the request filled up.
			 */
			if (now < ep->iso_next_ns)
				break;
			if (!ep->iso_next_ns || now > ep->iso_next_ns + 8 * interval)
				ep->iso_next_ns = now;
			ep->iso_next_ns += interval;
			iso = &urb->iso_frame_desc[urbp->iso_packet++];
			iso->actual_length = 0;
			iso->status = -ENOSR;
			if (!list_empty(&ep->queue)) {
				req = list_first_entry(&ep->queue, struct dummy_request, queue);
				len = min(iso->length, req->req.length - req->req.actual);
				if (len && urb->transfer_buffer && req->req.buf) {
					void *ubuf = urb->transfer_buffer + iso->offset;
					void *rbuf = req->req.buf + req->req.actual;
					if (usb_urb_dir_in(urb))
						memcpy(ubuf, rbuf, len);
					else
						memcpy(rbuf, ubuf, len);
				...
				iso->status = len == iso->length ? 0 : -EOVERFLOW;
				req->req.status = 0;
				list_del_init(&req->queue);
				spin_unlock(&dum->lock);
				usb_gadget_giveback_request(&ep->ep, &req->req);
			...
			if (iso->status)
				urb->error_count++;
			if (urbp->iso_packet == urb->number_of_packets)
				status = 0;
			break;
		}
...
static struct hc_driver dummy_hcd = {
	...
	.product_desc =		"Vibeshine DualSense host controller",
